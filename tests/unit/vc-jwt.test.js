...
import { decodeJwt, signCredential, verifyCredential } from "../../lib/vc.js";
...
describe("VC-JWT — tamper detection", () => {
	it("flipping a bit in the header segment fails verification with signature_invalid", async () => {
		...
		const tamperedH = Buffer.from(JSON.stringify(headerObj)).toString(
			"base64url",
		);
	...
	it("mutating the payload segment fails verification with signature_invalid", async () => {
		...
		const tamperedP = Buffer.from(JSON.stringify(payloadObj)).toString(
			"base64url",
		);
