...
describe("normalizeEmail", () => {
	...
	it("strips dots in the local-part for gmail.com and googlemail.com", () => {
		...
		assert.equal(
			normalizeEmail("a.l.i.c.e@googlemail.com"),
	...
	it("preserves dots in non-gmail local-parts", () => {
		assert.equal(
			normalizeEmail("a.l.i.c.e@example.com"),
	...
	it("combines +tag stripping and gmail dot stripping", () => {
		assert.equal(normalizeEmail("Al.Ice+SignUp@Gmail.com"), "alice@gmail.com");
	});
...
describe("signPayload / verifyPayload", () => {
	...
	it("returns null when the body has been tampered with", () => {
		...
		const flipped =
			body.charAt(0) === "A" ? `B${body.slice(1)}` : `A${body.slice(1)}`;
