...
import { ed25519Sign, ed25519Verify, pubKeyFromRaw, pubKeyRawBytes } from "../../lib/did.js";
  ...
  for (const v of vectors) {
    describe(v.name, () => {
      ...
      it("derives the spec-correct public key from the seed", () => {
        ...
        assert.equal(Buffer.from(raw).toString("hex"), expectedPub.toString("hex"));
      });
      ...
      it("verifies the spec signature against the spec public key", () => {
        assert.equal(ed25519Verify(message, expectedSig, publicKey), true);
      });
      ...
      it("rejects a signature with one bit flipped", () => {
        ...
        tampered[0] ^= 0x01;
        assert.equal(ed25519Verify(message, tampered, publicKey), false);
      });

      it("rejects the spec signature against a different message", () => {
        const wrongMsg = Buffer.concat([message, Buffer.from([0x00])]);
        assert.equal(ed25519Verify(wrongMsg, expectedSig, publicKey), false);
      });
    });
  }
});
