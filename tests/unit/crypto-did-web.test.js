...
describe("didWebFor — DID construction from app URL", () => {
  it("https origin without port → did:web:<host>", () => {
    assert.equal(didWebFor("https://rideshare.example.com"), "did:web:rideshare.example.com");
  });

  it("http origin without port → did:web:<host> (port-less form)", () => {
    assert.equal(didWebFor("http://example.com"), "did:web:example.com");
  });

  it("origin with non-default port encodes ':' as %3A", () => {
    assert.equal(didWebFor("http://localhost:9999"), "did:web:localhost%3A9999");
    assert.equal(didWebFor("https://example.com:8443"), "did:web:example.com%3A8443");
  });

  it("default ports (80 for http, 443 for https) are omitted", () => {
    assert.equal(didWebFor("http://example.com:80"), "did:web:example.com");
    assert.equal(didWebFor("https://example.com:443"), "did:web:example.com");
  });
  ...
  it("path on the URL is intentionally ignored (deployment-root only)", () => {
    ...
    assert.equal(didWebFor("https://example.com/some/sub/path"), "did:web:example.com");
  });
});
