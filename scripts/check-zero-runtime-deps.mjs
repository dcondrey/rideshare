...
try {
	...
	console.error(
		`[zero-runtime-deps] could not read ${PKG_PATH}: ${err.message}`,
	);
	process.exit(2);
}
...
if (violations.length > 0) {
	console.error(
		"[zero-runtime-deps] FAIL: this project must ship with zero runtime dependencies.",
	);
	console.error("");
	for (const { bucket, names } of violations) {
		console.error(
			`  - package.json#${bucket} is non-empty: ${names.join(", ")}`,
		);
	}
	console.error("");
	console.error(
		"If a tool is dev-only (linter, typechecker, test runner, build script),",
	);
	console.error(
		"move it to `devDependencies`. If it is genuinely required at runtime,",
	);
	console.error(
		"open an RFC issue first — adding a runtime dep changes the threat model.",
	);
	process.exit(1);
}
