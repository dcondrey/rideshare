...
try {
	...
	console.error(
		`[insert-readme-badges] could not read ${README_PATH}: ${err.message}`,
	);
	process.exit(2);
}
...
if (startIdx !== -1 && endIdx !== -1 && endIdx > startIdx) {
	...
	if (next === readme) {
		console.log(
			"[insert-readme-badges] no changes needed (badges already up to date).",
		);
		process.exit(0);
	}
...
if (h1Index === -1) {
	...
	writeFileSync(README_PATH, next);
	console.log(
		"[insert-readme-badges] no H1 found; inserted badges at the top.",
	);
	process.exit(0);
}
...
writeFileSync(README_PATH, next);
console.log(
	`[insert-readme-badges] inserted badges after H1 on line ${h1Index + 1}.`,
);
