...
export function claimsByUser(userId) {
  return /** @type {(ClaimRow & { airport: string, direction: string, depart_date: string, depart_time: string, kind: string, ride_status: string, poster_email: string, poster_name: string | null, poster_contact: string | null })[]} */ (
    db
      .prepare(
...
export function createClaim(input) {
  return tx(() => {
    const ride = /** @type {{ id: number, user_id: number, status: string } | undefined} */ (
    ...
    const r = db
      ...
      .run(input.rideId, input.claimerId, input.seats, input.message, Date.now());
    return Number(r.lastInsertRowid);
  });
}
