 ...
 *   (k-anonymity heuristic). This applies to every grouped cut, not just the
 *   airport one: the admin dashboard states the guarantee for all of them.
 */
...
/** @typedef {{ airport: string, kind: string, c: number }} AirportCount */
...
export function funnel() {
  ...
  const signedIn = /** @type {{c:number}} */ (db.prepare("SELECT COUNT(*) AS c FROM users").get())
    .c;
  const posters = /** @type {{c:number}} */ (
    db.prepare("SELECT COUNT(DISTINCT user_id) AS c FROM rides").get()
  ).c;
...
export function activity() {
  ...
  const acceptedClaims = scalar("SELECT COUNT(*) FROM claims WHERE status = 'accepted'");
  const requestRows = scalar("SELECT COUNT(*) FROM rides WHERE kind = 'request'");
...
export function byDirection() {
  const rows = /** @type {{ direction: string, kind: string, c: number }[]} */ (
    db
      .prepare(
  ...
  return coalesceSmall(rows, ["direction"], MIN_CELL);
}

export function byDate() {
  const rows = /** @type {{ date: string, kind: string, c: number }[]} */ (
    db
      .prepare(
  ...
  return coalesceSmall(rows, ["date"], MIN_CELL);
}
...
export function unmetDemand() {
  const rows = /** @type {{ date: string, airport: string, direction: string, c: number }[]} */ (
    db
      .prepare(
  ...
  // The finest cut in the file — date + airport + direction — so a count of one
  // is one attendee's ride post. This is also the row that leaves the building,
  // via /admin/insights.csv.
  return coalesceSmall(rows, ["date", "airport", "direction"], MIN_CELL);
}
...
export function metricsCsv() {
  ...
  const lines = [
    ...
    ...unmetDemand().map((r) => `${r.date},${esc(r.airport)},${r.direction},${r.c}`),
  ];
  return `${lines.join("\n")}\n`;
}
...
function scalar(sql, args = []) {
  const row = /** @type {Record<string, unknown> | undefined} */ (db.prepare(sql).get(...args));
  if (!row) return 0;
