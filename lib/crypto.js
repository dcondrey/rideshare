 ...
 *
 * IMPORTANT: compare the encoded buffers, not the strings. `String.length`
 * counts UTF-16 code units while `Buffer.from` encodes UTF-8, so a multibyte
 * input can pass a string-length check and then make `timingSafeEqual` throw
 * RangeError on the byte-length mismatch — turning a failed comparison on an
 * attacker-supplied value into a 500.
 * @param {string} a
 * @param {string} b
 */
