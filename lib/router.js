...
function securityHeaders() {
	return {
		...
		"Content-Security-Policy":
			...
			"default-src 'self'; img-src 'self' data: https:; style-src 'self'; script-src 'self'; connect-src 'self'; form-action 'self'; base-uri 'self'; frame-ancestors 'none'",
	};
}
...
function parseUrlEncoded(s) {
	...
	for (const part of s.split("&")) {
		...
		const k = decodeURIComponent(
			(eq === -1 ? part : part.slice(0, eq)).replace(/\+/g, " "),
		);
		const v =
			eq === -1
				? ""
...
function handleError(ctx, err) {
	...
	// ctx.method/ctx.pathname originate from the request line and are attacker-
	// controlled. Strip CR/LF so a crafted request can't forge extra log lines,
	// and pass them as %s substitution args (not as/in the format string
	// itself) so tainted data can never be interpreted as a format directive.
	const logSafe = (value) => String(value).replace(/[\r\n]/g, "");
	console.error("[%s %s]", logSafe(ctx.method), logSafe(ctx.pathname), err);
