 ...
 *   - max 200KB
 *   - mime must be one of: image/png, image/webp, image/jpeg
 *
 * SVG is deliberately NOT accepted. It is an executable document, /logo is
 * unauthenticated and is rendered on the public landing page, and the sanitiser
 * THREAT_MODEL.md CC-6 describes does not exist in this codebase. A raster logo
 * costs an organizer nothing; a stored same-origin script costs every visitor.
 */

import { audit, db } from "./db.js";

export const MAX_LOGO_BYTES = 200 * 1024;
export const ALLOWED_LOGO_MIMES = new Set(["image/png", "image/webp", "image/jpeg"]);
...
export function getAsset(name) {
  const row = /** @type {{ mime_type: string, bytes: Buffer, updated_at: number } | undefined} */ (
    db.prepare("SELECT mime_type, bytes, updated_at FROM assets WHERE name = ?").get(name)
  );
  if (!row) return null;
  return {
    mime: row.mime_type,
...
export function hasLogo() {
  const r = db.prepare("SELECT 1 FROM assets WHERE name = 'logo'").get();
  return !!r;
}
