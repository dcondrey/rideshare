...
/**
 * @typedef {{ id: number, algorithm: string, public_key_jwk: string,
 *   private_key_jwk: string, did: string, key_fragment: string,
 *   created_at: number }} SigningKeyRow
...
export function getDeploymentKey() {
	...
	const row = /** @type {SigningKeyRow | undefined} */ (
	...
	console.info(`[trust] generated deployment Ed25519 keypair; DID = ${did}`);
...
export function confirmRide(args) {
	...
	const ride = /** @type {{ id: number, poster_id: number } | undefined} */ (
		db
			.prepare("SELECT id, user_id AS poster_id FROM rides WHERE id = ?")
			.get(args.rideId)
	);
...
function runIssuanceForRide(rideId) {
	...
	if (!ride)
		return { recorded: false, dualConfirmed: false, issuedCredentialIds: [] };
...
export async function importCredential(args) {
	...
	const subjectDid =
		parts.payload.sub || parts.payload.vc?.credentialSubject?.id;
	if (subjectDid !== myDid) {
		return {
			ok: false,
	...
	const credentialId =
		parts.payload.jti || `urn:hash:${parts.signature.slice(0, 32)}`;
	...
	const counterpartDid =
		parts.payload.vc?.credentialSubject?.counterpart || null;
...
export function trustProfileFor(userId) {
	const issuedHere =
		/** @type {{ subject_did: string, counterpart_did: string | null }[]} */ (
			db
				.prepare(
	...
	const imported =
		/** @type {{ issuer_did: string, counterpart_did: string | null }[]} */ (
			db
				.prepare(
