...
import { ed25519Sign, ed25519Verify, pubKeyFromRaw, resolveDid } from "./did.js";
...
export function signCredential(args) {
  ...
  const expSec = args.expirationDate ? Math.floor(args.expirationDate.getTime() / 1000) : undefined;
  ...
  const vc = {
    "@context": ["https://www.w3.org/ns/credentials/v2", "https://eventrideshare.org/contexts/v1"],
...
export function decodeJwt(jwt) {
  ...
  // JSON.parse happily yields null, a number or an array. Every caller then
  // reads properties off these, so reject anything that is not a plain object
  // here rather than throwing a TypeError out of the middle of verification.
  if (!isPlainObject(header)) throw new Error("JWT header is not an object");
  if (!isPlainObject(payload)) throw new Error("JWT payload is not an object");
  return {
    header,
    payload,
    signature: parts[2],
...
export async function verifyCredential(jwt, opts = {}) {
  ...
  try {
    ...
    return {
      ok: false,
      errors: [`malformed_jwt: ${errorMessage(err)}`],
      checks,
    };
  }
  ...
  const issuer = payload.iss || payload.vc?.issuer;
  ...
  // signCredential writes sub and vc.credentialSubject.id from one value, so the
  // signer commits to their equality and the verifier has to enforce it. Without
  // this, a credential reading sub=VICTIM / credentialSubject.id=ATTACKER
  // verifies here while any other VCDM reader of the same JWT disagrees about
  // who it is about. Same argument for jti against vc.id.
  const vcSubject = payload.vc?.credentialSubject?.id;
  if (payload.sub && vcSubject && payload.sub !== vcSubject) {
    errors.push(`subject_mismatch: jwt.sub ${payload.sub} vs vc.credentialSubject.id ${vcSubject}`);
  }
  if (payload.jti && payload.vc?.id && payload.jti !== payload.vc.id) {
    errors.push(`vc.id_mismatch: jwt.jti ${payload.jti} vs vc.id ${payload.vc.id}`);
  }
  ...
  if (issuer && errors.length === 0) {
    ...
    try {
      ...
      return {
        ok: false,
        errors: [`issuer_resolution_failed: ${errorMessage(err)}`],
        checks,
    ...
    let sigOk = false;
    try {
      // pubKeyFromRaw rejects a key that is not 32 raw bytes, which a hostile or
      // simply wrong DID document can easily contain; that belongs inside the
      // same guard as the verify itself.
      const pubKey = pubKeyFromRaw(rawPubKey);
      sigOk = ed25519Verify(Buffer.from(signingInput, "ascii"), b64uDecode(signature), pubKey);
    } catch (err) {
      errors.push(`signature_verify_threw: ${errorMessage(err)}`);
    }
