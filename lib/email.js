 ...
 * Retries transient failures (network, timeout) up to 2 times with ~500ms backoff.
 * @param {EmailMessage} msg
 */
export async function sendEmail(msg) {
  if (config.resendApiKey) return sendWithRetry(() => sendViaResend(msg));
  if (config.smtp.host) return sendWithRetry(() => sendViaSmtp(msg));
  throw new Error("No email transport configured. Set RESEND_API_KEY or SMTP_HOST.");
}

/**
 * Retry helper: retries on transient errors (network, timeout).
...
async function sendWithRetry(fn) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      logWarn("transient email send failure, retrying", { component: "email", attempt, err });
      if (attempt < 1) await new Promise((r) => setTimeout(r, 500 + Math.random() * 500));
    }
  }
  throw lastErr;
}

const PAST_NO_RETURN = Symbol.for("rideshare.email.pastPointOfNoReturn");

/**
 * Tag an error raised after the message body was handed to the relay, so the
 * retry loop treats it as permanent whatever the message says.
 * @param {unknown} err
 */
function markPastPointOfNoReturn(err) {
  const e = err instanceof Error ? err : new Error(String(err));
  /** @type {Record<symbol, boolean>} */ (/** @type {unknown} */ (e))[PAST_NO_RETURN] = true;
  return e;
}

/**
 * Classify if an error is transient (retriable) or permanent (fail-fast).
 * @param {unknown} err
 */
function isTransientError(err) {
  if (err && /** @type {Record<symbol, boolean>} */ (err)[PAST_NO_RETURN]) return false;
  const msg = err instanceof Error ? err.message : "";
  return /timeout|ECONNREFUSED|ENOTFOUND|EHOSTUNREACH|ENETUNREACH|aborted/i.test(msg);
}
...
async function sendViaSmtp(msg) {
  const { host, port, user, pass, secure, allowPlaintext } = config.smtp;
  ...
  sock.setTimeout(10000, () => {
    sock.destroy(new Error("SMTP socket timeout"));
  });
  ...
  let { lines } = await session.expect(250);

  // STARTTLS upgrade. REQUIRED whenever the connection did not start out with
  // implicit TLS: a relay that does not advertise the upgrade is refused rather
  // than continued in the clear, because the AUTH below would otherwise put the
  // credentials on a plaintext socket, and an active attacker can strip the
  // advertisement from the EHLO reply to force exactly that.
  if (!secure) {
    ...
    if (!offersStartTls) {
      if (!allowPlaintext) {
        sock.destroy();
        throw new Error(
          `SMTP server ${host}:${port} does not offer STARTTLS; refusing to continue unencrypted. ` +
            `Use SMTP_SECURE=true for implicit TLS, or set SMTP_ALLOW_PLAINTEXT=true for a local relay.`,
        );
      }
    } else {
      ...
      session.rebind(sock);
      await session.cmd(`EHLO ${safeHostname()}`);
      ({ lines } = await session.expect(250));
    }
  }
  ...
  // IMPORTANT: past this point the relay may already have taken the message.
  // A socket timeout waiting for the final 250 matches isTransientError's
  // /timeout/i, and retrying would open a second session with the same body —
  // the attendee gets two magic links. Anything thrown from here is marked
  ...
  try {
    ...
    throw markPastPointOfNoReturn(err);
  }
...
function upgradeToTls(sock, host) {
  return new Promise((resolve, reject) => {
    const tlsSock = tlsConnect({ socket: sock, servername: host }, () => resolve(tlsSock));
...
class SmtpSession {
  ...
  /**
   * Point the session at the socket produced by the STARTTLS upgrade.
   *
   * IMPORTANT: anything the server sent before the upgrade is discarded, and the
   * pre-TLS socket's listeners are detached. Carrying either across the upgrade
   * is what lets a plaintext server inject responses that are then read as
   * though they had arrived over the encrypted channel.
   * @param {import("node:net").Socket} sock
   */
  rebind(sock) {
...
function encodeHeader(s) {
  // Pure-ASCII headers go out verbatim. Checked by code point rather than a
  // regex, which would have to spell out the control-character range.
  let ascii = true;
