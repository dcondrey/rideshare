...
// Upper bound on any base58btc input. The codec is general-purpose (the largest
// legitimate payload here is a DID document's publicKeyMultibase), but the decode
// is quadratic, so an absurd length is refused outright.
const MAX_BASE58_CHARS = 1024;
const DID_WEB_TIMEOUT_MS = 5000;
// A DID document is a handful of keys and service entries. Anything larger is a
// hostile or broken issuer, and res.json() would buffer all of it.
const MAX_DID_DOC_BYTES = 64 * 1024;
// 2-byte multicodec + 32-byte Ed25519 public key encodes to exactly 47 chars.
const ED25519_DID_KEY_CHARS = 47;
const BASE58_INDEX = (() => {
 ...
 *
 * Checked while streaming rather than from Content-Length alone, because a
 * chunked response can declare nothing and still never end.
 * @param {Response} res
 * @param {number} limit
...
async function readCapped(res, limit) {
  ...
  if (Number.isFinite(declared) && declared > limit) {
    throw new Error(`DID document too large: ${declared} > ${limit} bytes`);
  }
  if (!res.body) return "";
  ...
  for await (const chunk of res.body) {
    total += chunk.length;
    if (total > limit) throw new Error(`DID document too large: exceeds ${limit} bytes`);
 ...
 *
 * IMPORTANT: the decode is O(n^2) in the input length, and callers reach it with
 * strings taken from unauthenticated request bodies, so the length is bounded
 * here rather than at each call site.
 */
export function base58btcDecode(str) {
  if (str.length === 0) return new Uint8Array(0);
  if (str.length > MAX_BASE58_CHARS) {
    throw new Error(`base58 string too long: ${str.length} > ${MAX_BASE58_CHARS}`);
  }
  let zeroes = 0;
...
export function didKeyToPubKey(didKey) {
  ...
  // An Ed25519 did:key is always the 2-byte multicodec plus 32 key bytes, which
  // encodes to exactly this many characters. Checking before the decode keeps an
  // attacker-supplied DID from paying for the quadratic decode at all.
  if (m[1].length !== ED25519_DID_KEY_CHARS) {
    throw new Error(`did:key is not Ed25519 (expected ${ED25519_DID_KEY_CHARS} base58 chars)`);
  }
  ...
  if (
    ...
    throw new Error(`did:key is not Ed25519 (multicodec mismatch or wrong length)`);
  }
 ...
 *
 * @param {string} did
 * @returns {Promise<{ rawPubKey: Uint8Array, didDocument?: Record<string, unknown> }>}
 */
export async function resolveDid(did) {
  ...
  if (did.startsWith("did:web:")) {
    ...
    if (u2.protocol !== "https:") {
      const allowDevHttp =
        config.allowInsecureDidWeb && /^(localhost|127\.0\.0\.1)$/.test(u2.hostname);
      if (!(u2.protocol === "http:" && allowDevHttp)) {
        throw new Error(`did:web must be HTTPS: ${url}`);
      }
    }
    // A peer deployment publishes its DID document on 443. An attacker-chosen
    // port turns the verifier's error text into a port scanner for public
    // hosts, so refuse one outright rather than relying on the egress policy.
    if (u2.protocol === "https:" && u2.port && u2.port !== "443") {
      throw new Error(`did:web must use port 443: ${url}`);
    }
    let raw;
    if (u2.protocol === "https:") {
      raw = (
        await safeFetch(url, {
          ...
          contentType: /^application\/(did\+)?json\b/i,
          maxBytes: MAX_DID_DOC_BYTES,
          timeoutMs: DID_WEB_TIMEOUT_MS,
        })
      ).body;
    } else {
      // ALLOW_INSECURE_DID_WEB only: plaintext loopback, deliberately outside
      // the egress policy because that policy refuses both http and loopback.
      const res = await fetch(url, {
        headers: { Accept: "application/did+json, application/json" },
        redirect: "error",
        signal: AbortSignal.timeout(DID_WEB_TIMEOUT_MS),
      });
      if (!res.ok) throw new Error(`Failed to fetch ${url}: ${res.status}`);
      raw = await readCapped(res, MAX_DID_DOC_BYTES);
    }
    const doc = JSON.parse(raw);
    if (doc.id !== did) {
      throw new Error(`DID document id (${doc.id}) does not match requested DID (${did})`);
    }
