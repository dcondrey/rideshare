...
function loadEventConfig() {
  ...
  console.error(`[config] No event.config.yaml (or .yml or .json) found in ${ROOT}`);
  process.exit(1);
}
...
if (adminEmails.length === 0) {
  console.warn("[config] WARNING: ADMIN_EMAILS is empty — nobody will be able to access /admin.");
}
...
if (config.sessionSecret.length < 32) {
  console.warn("[config] WARNING: SESSION_SECRET is short. Use 32+ random hex bytes.");
}
if (config.allowlistSalt.length < 32) {
  console.warn("[config] WARNING: ALLOWLIST_SALT is short. Use 32+ random hex bytes.");
}
