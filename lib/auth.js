...
import { hmac, normalizeEmail, randomToken } from "./crypto.js";
import { audit, db, tx } from "./db.js";
import { sendEmail } from "./email.js";
import { getEventConfig } from "./event-config.js";
import { rateLimit } from "./rate-limit.js";

// ADMIN_EMAILS is operator-typed, so it is only trimmed and lowercased in
// lib/config.js, while users.email was written through normalizeEmail (which
// also strips +tags, and dots on gmail). Comparing the two shapes directly
// silently denies /admin to any admin whose address has either. Both sides go
// through the same normalizer here, at the one place the list is consumed.
const ADMIN_EMAILS = new Set(config.adminEmails.map(normalizeEmail));

/**
 * @typedef {{ expires_at: number, id: number, email: string,
 *   display_name: string | null, contact_method: string | null }} SessionUserRow
 */
...
export async function startMagicLink(rawEmail, ip) {
  ...
  const now = Date.now();
  // IMPORTANT: requesting a link invalidates every outstanding one for this
  // address, so a link leaked from an older email cannot still be redeemed.
  // Marking used (rather than deleting) keeps consumeMagicLink's "Link already
  // used" message instead of the generic "Invalid or expired link".
  tx(() => {
    db.prepare("UPDATE magic_links SET used_at = ? WHERE email = ? AND used_at IS NULL").run(
      now,
      email,
    );
    db.prepare(
...
export function consumeMagicLink(rawToken, ip, userAgent) {
  ...
  const row =
    /** @type {{ email: string, expires_at: number, used_at: number | null } | undefined} */ (
      db
  ...
  const now = Date.now();
  db.prepare("UPDATE magic_links SET used_at = ? WHERE token_hash = ?").run(now, tokenHash);
  ...
  // Upsert user
  let user = /** @type {{ id: number, email: string } | undefined} */ (
    db.prepare("SELECT id, email FROM users WHERE email = ?").get(row.email)
  );
  if (!user) {
    const r = db
      .prepare(`INSERT INTO users (email, created_at, last_seen_at) VALUES (?, ?, ?)`)
...
export function getCurrentUser(req) {
  ...
  const row = /** @type {SessionUserRow | undefined} */ (
    db
      .prepare(
  ...
  return {
    ...
    isAdmin: ADMIN_EMAILS.has(normalizeEmail(row.email)),
  };
}
...
/**
 * Parse a Cookie header into a null-prototype map.
 * @param {string} cookieHeader
 */
export function parseCookie(cookieHeader) {
  // Cookie names are attacker-controlled; use a null-prototype object so a
  // name like "__proto__" can't be interpreted as a prototype write.
