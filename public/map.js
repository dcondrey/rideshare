		...
		c.addEventListener("pointermove", (e) => {
			...
			if (ids.length === 1 && panAnchor) {
				...
				if (targetZoom !== self.zoom) {
					...
					var rect = c.getBoundingClientRect();
					var _midX = (a.x + b.x) / 2 - rect.left;
					var _midY = (a.y + b.y) / 2 - rect.top;
					self.zoomBy(0); // no-op
		...
		(data.meetups || []).forEach((m) => {
			map.addMarker({
				...
				html:
					...
					"</strong>" +
					(m.address
						? `<br><span class="muted">${escapeHtml(m.address)}</span>`
						: "") +
